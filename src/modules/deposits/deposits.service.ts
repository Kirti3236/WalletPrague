...
export class DepositsService {
  ...
  async depositFromCard(dto: DepositFromCardDto) {
    return this.sequelize.transaction(async (tx) => {
      ...
      const wallet = await (Wallet as any).findByPk(dto.wallet_id, { transaction: tx, lock: tx.LOCK.UPDATE });
      ...
      const policy = await FeePolicy.findByPk('deposit_fee_card', { transaction: tx });
      const feeFlat = parseFloat((policy?.amount as unknown as string) || '0.00');
      ...
      const txn = await Transaction.create(
        {
          ...
          status: 'completed',
          description: dto.description || `Card deposit from *${paymentMethod.last4}`,
      ...
      const currentBalance = parseFloat(wallet.available_balance as unknown as string);
      wallet.available_balance = (currentBalance + amount).toFixed(2) as unknown as string;
      wallet.ledger_balance = (currentBalance + amount).toFixed(2) as unknown as string;
  ...
  async depositFromBank(dto: DepositFromBankDto) {
    return this.sequelize.transaction(async (tx) => {
      ...
      // Get deposit fee policy (if any)
      const policy = await FeePolicy.findByPk('deposit_fee_bank', { transaction: tx });
      const feeFlat = parseFloat((policy?.amount as unknown as string) || '0.00');
      ...
      const txn = await Transaction.create(
        {
          ...
          net_amount: (amount - feeFlat).toFixed(2),
          status: 'completed',
          description: dto.description || `Bank deposit from ${paymentMethod.bank_name}`,
          processed_at: new Date(),
        } as any,
        { transaction: tx },
      );
