...
const emailTemplateFile = path.join(__dirname, 'email-template-with-reference.html');
const resumeFile = path.join(__dirname, 'Hardik Sheladiya_Node JS_Resume.pdf');

// Get target email from command line argument or use default
const TARGET_EMAIL = process.argv[2] || 'hr@questglt.org';
...
Reference by: Pradip Bambharoliya
 ...
 */
async function main() {
  console.log(`\n📧 Sending email with reference to: ${TARGET_EMAIL}\n`);
  ...
  if (result.success) {
    ...
    console.log(`   Reference: Pradip Bambharoliya`);
  } else {
    console.log('\n❌ Email failed!');
    process.exit(1);
  }
